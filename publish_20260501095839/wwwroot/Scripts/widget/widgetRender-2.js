...
// Safely walk up the parent chain to find GetStudentID()
function getStudentID() {
    let win = window;
    while (win) {
        try {
            if (typeof win.GetStudentID === 'function') {
                return win.GetStudentID();
            }
            ...
            // Cross-origin frame blocked — stop here
            console.warn("Cross-origin frame encountered, stopping parent traversal.");
            break;
        }
    }
    console.warn("GetStudentID() not found. Defaulting to 1.");
    return 1;
}
...
const widgetHandler = (function () {
    ...
    async function saveUserFirstResponse_txt(btn) {
        try {
            ...
            let responseText = responseElement.value.trim();
            ...
            document.getElementById('bfr_resp_msg_' + widgetId).style.display = "block"; // ← bug fix
    ...
    return {
        loadWidget,
        saveUserFirstResponse_txt,
        saveUserLastResponse_txt
    };
})();