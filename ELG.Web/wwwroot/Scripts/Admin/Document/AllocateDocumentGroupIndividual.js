$(function () {
    ...
    UTILS.activateMenuNavigationLink('menu-docgroup-report');
        allocateIndividalDocumentHandler.init();
    $('[data-toggle="tooltip"]').tooltip();
});


var allocateIndividalDocumentHandler = (function () {
    ...
    function showDefaultMessage() {
        ...
        var message = '<div > <b>How to use this page:</b> <ul>' +
            ...
            '<li>Click <b>Allocate to Selected</b> to assign the document to selected learners.</li>' +
            '<li>Use the <i class="fa fa-ellipsis-v me-1"></i> menu in each row to assign or revoke access individually.</li>' +
            '</ul></div>';
    ...
    function resetSelections() {
        ...
        $allocateAllBtn.removeAttr("onclick", "allocateIndividalDocumentHandler.assignToSelected()");
    ...
    $searchBtn.click(function (e) {
        ...
        $('#allocateIndividualDocumentLearnerList').DataTable({
            ...
            drawCallback: function () {
                allocateIndividalDocumentHandler.selectAllRows();
            ...
            columns: [
                { data: "UserID", name: "UserID", width: "5%" },
                { data: "FirstName", name: "strFirstName", width: "20%" },
                { data: "EmailId", name: "strEmail", width: "25%" },
                { data: "Location", name: "strLocation", width: "20%" },
                { data: "Department", name: "strDepartment", width: "20%" },
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.IsCourseExpired) {
                            return `<input type="checkbox" class="form-check-input" id="chk-user-${row.UserID}" name="chk-user-${row.UserID}" value="${safeId}" disabled>`;
                        } else {
                            ...
                            return `
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown-${id}">
                                        ${isExpired
                                                ? `<li>
                                                <a class="dropdown-item text-danger" href="#" id="revoke-access-${id}"
                                                   onclick="allocateIndividalDocumentHandler.revokeIndividualDocumentAccess(this)">
                                                    <i class="fa fa-minus-circle me-2"></i>Revoke Access
                                                </a>
                                               </li>`
                                                : `<li>
                                                <a class="dropdown-item" href="#" id="assign-access-${id}"
                                                   onclick="allocateIndividalDocumentHandler.assignIndividualDocumentAccess(this)">
    ...
    //function to revoke Document to single learner and refresh table
    function revokeIndividualDocumentAccess(btn) {
        var groupName = decodeURIComponent($.urlParam('group'));
        if (confirm("Are you sure you want to revoke " + groupName+" access from this user?")) {
            var learnerId = btn.id.split('-').pop();
            var url = hdnBaseUrl + 'Document/RevokeIndividualDocumentToLearner'
            var data = {
                LearnerID: learnerId,
            ...
            UTILS.makeAjaxCall(url, data, function (result) {
                if (result.success == 1) {
                    UTILS.Alert.show($alert, 'success', 'Document group access revoked successfully.')
                    $('#allocateIndividualDocumentLearnerList').DataTable().draw();
                }
                else
                    UTILS.Alert.show($alert, 'error', 'Failed to revoke document group access.')
            }, function (status) {
                console.log(status);
                });
        }
    ...
    function assignIndividualDocumentAccess(btn) {
        var groupName = decodeURIComponent($.urlParam('group'));
        if (confirm("Are you sure you want to assign " + groupName+" to this user?")) {
            var learnerId = btn.id.split('-').pop();
            var url = hdnBaseUrl + 'Document/AllocateIndividualDocumentGroupToLearner?groupID=' + selectedGroup
            var data = {
                LearnerID: learnerId
            }
            UTILS.makeAjaxCall(url, data, function (result) {
    ...
    $selectAllChkBox.on('click', function () {
        ...
        if (this.checked) {
            ...
            $allocateAllBtn.attr("onclick", "allocateIndividalDocumentHandler.assignToSelected()");
    ...
    function selectAllRows() {
        ...
        $('input[type="checkbox"].chk-user', rows).prop('checked', $selectAllChkBox.prop('checked'));
    ...
    function assignToSelected() {
        if (selUsers.length > 0 || selectAllUsers) {
            ...
            if (confirm("Are you sure you want to assign " + groupName+" access to all selected users?")) {
                ...
                var url = hdnBaseUrl + "Document/AllocateIndividualDocumentGroupToLearner_Multiple?groupID=" + selectedGroup;
                var data = {
                ...
                UTILS.makeAjaxCall(url, data, function (result) {
                    ...
                    console.log(status);
                    UTILS.Alert.show($alert, 'error', 'Failed to assign document group. Please try again later');
                    UTILS.resetButton($allocateAllBtn);
                })
            ...
            UTILS.Alert.show($alert, 'error', 'Please select learner(s) to allocate document group');
            resetSelections()
    ...
    return {
        ...
        revokeIndividualDocumentAccess: revokeIndividualDocumentAccess,
