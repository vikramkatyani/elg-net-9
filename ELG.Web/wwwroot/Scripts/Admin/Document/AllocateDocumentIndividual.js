...
var allocateIndividalDocumentHandler = (function () {
    ...
    $searchBtn.click(function (e) {
        ...
        $('#allocateIndividualDocumentLearnerList').DataTable({
            ...
            ajax: {
                url: hdnBaseUrl + "Document/LoadLearnerDataToAllocateIndividalDocument",
                type: "POST",
                datatype: "json",
                data: function (data) {
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        //const btnClass = isExpired ? "btn-danger" : "btn-success";
                        const iconClass = isExpired ? "fa-minus-circle" : "fa-plus-circle";
                        const btnText = isExpired ? "Revoke Access" : "Assign Access";
                        const actionFn = isExpired ? "revokeIndividualDocumentAccess" : "assignIndividualDocumentAccess";

                        return `
                            <button type="button" data-toggle="tooltip" title = "${btnText}"
                                    class="btn btn-sm  border-0 p-2 rounded-circle"
                                    id="${isExpired ? `revoke-access-${id}` : `assign-access-${id}`}"
                                    onclick="allocateIndividalDocumentHandler.${actionFn}(this)">
    ...
    function revokeIndividualDocumentAccess(btn) {
        var docName = decodeURIComponent($.urlParam('doc'));
        if (confirm("Are you sure you want to revoke " + docName+" access from this user?")) {
            var learnerId = btn.id.split('-').pop();
            var url = hdnBaseUrl + 'Document/RevokeIndividualDocumentToLearner'
            var data = {
                LearnerID: learnerId,
                Course: selectedDocument
            }
            UTILS.makeAjaxCall(url, data, function (result) {
                if (result.success == 1) {
                    UTILS.Alert.show($alert, 'success', 'Document access revoked successfully.')
                    $('#allocateIndividualDocumentLearnerList').DataTable().draw();
                }
                else
                    UTILS.Alert.show($alert, 'error', 'Failed to revoke document access.')
            }, function (status) {
                console.log(status);
                });
        }
    ...
    function assignIndividualDocumentAccess(btn) {
        var docName = decodeURIComponent($.urlParam('doc'));
        if (confirm("Are you sure you want to assign " + docName+" to this user?")) {
            var learnerId = btn.id.split('-').pop();
            var url = hdnBaseUrl + 'Document/AllocateIndividualDocumentToLearner'
            var data = {
                LearnerID: learnerId,
                Course: selectedDocument
            }
            UTILS.makeAjaxCall(url, data, function (result) {
                if (result.success == 1) {
                    ...
                    UTILS.Alert.show($alert, 'error', 'Failed to assign document.')
            }, function (status) {
                console.log(status);
            });
        }
    ...
    function assignToSelected() {
        if (selUsers.length > 0 || selectAllUsers) {
            ...
            var docName = decodeURIComponent($.urlParam('doc'));
            if (confirm("Are you sure you want to assign " + docName+" access to all selected users?")) {
                // allocate functionality
                UTILS.disableButton($allocateAllBtn);
                var url = hdnBaseUrl + "Document/AllocateIndividualDocumentToLearner_Multiple";
                var data = {
